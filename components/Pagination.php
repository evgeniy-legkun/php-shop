...
class Pagination
{
    ...
    public function get()
    {
        ...
        $limits = $this->limits();
        //Тег открывающий список
        $html = '<ul class="pagination">';
        // Генерируем ссылки
        for ($page = $limits[0]; $page <= $limits[1]; $page++) {
            // Если текущая это текущая страница, ссылки нет и добавляется класс active
            if ($page == $this->current_page) {
                if($this->total > Product::SHOW_BY_DEFAULT) //добавляем навигацию если товаров > чем SHOW_BY_DEFAULT
    ...
    /*Возвращает общее число страниц страниц
     * @return число страниц
     */
    private function amount()
    {
        //Возвращаем значение округленное в большую сторону
