<?php include ROOT . '/views/layouts/admin_header.php'; ?>

<section>
    <div class="container">
        <div class="row">
            <div>
                <br />
                <a href="/admin/" class="btn btn-info">Админпанель</a>
                <a href="/admin/product/" class="btn btn-info">Управление товарами</a>
                <a href="/admin/product/update/<?php echo $id; ?>" class="btn btn-link">Редактировать товар</a>
            </div>
            <br /><br />

            <h3>Редактировать товар # <?php echo $id; ?></h3>
            <br />

            <div class="col-lg-4">
                <div class="login-form">
                    <form action="#" method="post" enctype="multipart/form-data">
                        <br/>
                        <h5>Название товара</h5>
                        <input type="text" name="name" placeholder="" value="<?php echo $product['name']; ?>"/>

                        <h5>Код товара</h5>
                        <input type="text" name="code" placeholder="" value="<?php echo $product['code']; ?>"/>

                        <h5>Стоимость, USD</h5>
                        <input type="number" name="price" placeholder="" value="<?php echo $product['price']; ?>"/>

                        <h5>Категория</h5>
                        <select name="category_id" title="Вибор категории товара">
                            <option></option>
                            <?php if (is_array($categoryList)): ?>
                                <?php foreach ($categoryList as $category): ?>
                                    <option value="<?php echo $category['id']; ?>"
                                        <?php if ($category['id'] == $product['category_id']) echo 'selected'?>>
                                        <?php echo $category['name']; ?>
                                    </option>
                                <?php endforeach;?>
                            <?php endif; ?>
                        </select>

                        <h5>Производитель</h5>
                        <input type="text" name="brand" placeholder="" value="<?php echo $product['brand']; ?>"/>

                        <h5>Картинка с товаром (размер 268х249)</h5>
                        <img src="<?php echo Product::getImage($product['id'])?>" width="200" alt="error">
                        <input type="file" name="image" placeholder="" value="<?php echo $product['image']; ?>"/>

                        <h5>Описание товара</h5>
                        <textarea name="description" title="Описание товара"><?php echo $product['description']; ?>
                        </textarea>

                        <h5>Наличие на складе</h5>
                        <select name="availability" title="Наличие на складе">
                            <option value="1" <?php if ($product['availability'] == 1) echo 'selected'?>>Да</option>
                            <option value="0" <?php if ($product['availability'] == 0) echo 'selected'?>>Нет</option>
                        </select>

                        <h5>Новинка</h5>
                        <select name="is_new" title="Новинка">
                            <option value="1" <?php if ($product['is_new'] == 1) echo 'selected'?>>Да</option>
                            <option value="0" <?php if ($product['is_new'] == 0) echo 'selected'?>>Нет</option>
                        </select>

                        <h5>Рекомендуемые</h5>
                        <select name="is_recommended" title="Рекомендуемые">
                            <option value="1" <?php if ($product['is_recommended'] == 1) echo 'selected'?>>Да</option>
                            <option value="0" <?php if ($product['is_recommended'] == 0) echo 'selected'?>>Нет</option>
                        </select>

                        <h5>Статус</h5>
                        <select name="status" title="Статус">
                            <option value="1" <?php if ($product['status'] == 1) echo 'selected'?>>Отображается</option>
                            <option value="0" <?php if ($product['status'] == 0) echo 'selected'?>>Скрыт</option>
                        </select>

                        <br /><br />
                        <button type="submit" name="submit" class="btn btn-info active">
                            <i class="fa fa-floppy-o"></i> Сохранить</button>
                        <br /><br />
                    </form>
                </div>
            </div>
        </div>
    </div>
</section>

<?php include ROOT . '/views/layouts/admin_footer.php'; ?>